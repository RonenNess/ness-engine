...
void Sprite3d::do_render(const Ness::Rectangle& target, const Ness::SRenderTransformations& transformations)
{
	...
	Ness::Size HalfSize = get_absolute_size_const() * 0.5f;
	for (unsigned int i = 0; i < lights.size(); i++)
	{
		...
		if (curr->get_position().y < get_position().y + HalfSize.y)
		{
			...
			if (CurrColor.a > 0.0f)
			{
				CurrColor.a *= abs(vector.y);
				printf("%f\r\n", vector.y);
				m_renderer->blit(m_light_top, &m_source_rect, target, Ness::BLEND_MODE_ADD, transformations.color * CurrColor, transformations.rotation, m_anchor);
			}
		}/*
		// if this light is left to this object most right side, check lighting from left side:
		if (curr->get_position().x < get_position().x + HalfSize.x)
		{
			m_renderer->blit(m_light_left, &m_source_rect, target, Ness::BLEND_MODE_ADD, transformations.color, transformations.rotation, m_anchor);
		}
		// if this light is right to this object most left side, check lighting from right side:
		if (curr->get_position().x > get_position().x - HalfSize.x)
		{
			m_renderer->blit(m_light_right, &m_source_rect, target, Ness::BLEND_MODE_ADD, transformations.color, transformations.rotation, m_anchor);
		}
		// if this light is below this object top, check lighting from bottom:
		if (curr->get_position().y > get_position().y - HalfSize.y)
		{
			m_renderer->blit(m_light_bottom, &m_source_rect, target, Ness::BLEND_MODE_ADD, transformations.color, transformations.rotation, m_anchor);
		}*/
	}
